                ...
                with col3:
                    if st.checkbox(item, key=f"{category}_{i}"):
                        ...
                        if quantity_type == "Below 1 kg":
                            ...
                            quantity = st.number_input("Quantity (kg)", min_value=1, max_value=20,
                                                       key=f"{category}_{i}_quantity")

                        cartitems.append((category, item, quantity))
                with col4:
                    st.image(image_path)
                with col5:
                    st.markdown(f"<h4 style='font-weight: bold; color: black;'>₹{price}</h4>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        for category, item_name, quantity in cartitems:
            ...
            with col2:
                st.image(image_path)
            with col3:
                st.markdown(f"<h4 style='font-weight: bold; color: black;'>Price: ₹{(price )} * {(quantity)} kg = ₹{round(price * quantity)}</h4>",
                            unsafe_allow_html=True)

        st.markdown(f"<h4 style='font-weight: bold; color: red;'>Total Price: ₹{total_price}</h4>", unsafe_allow_html=True)
